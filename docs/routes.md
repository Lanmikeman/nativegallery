...
| `/music` | Музыкальная библиотека и настройка потоков |

## Музыка (требуется авторизация)

| URL | Описание |
|-----|----------|
| `/music` | Библиотека: радио сайта, личные треки, потоки, плейлисты, M3U |

Мини-плеер в шапке доступен на всех страницах после входа. При SPA-переходах воспроизведение не прерывается.

### API музыки

| URL | Метод | Описание |
|-----|-------|----------|
| `/api/audio/library` | GET | Библиотека пользователя + `global_streams` |
| `/api/audio/upload` | POST | Загрузить аудиофайл |
| `/api/audio/stream` | POST | Добавить личный HTTP-поток |
| `/api/audio/url` | POST | Добавить прямую ссылку на файл |
| `/api/audio/playlist` | POST | Плейлисты (create, add, delete, reorder) |
| `/api/audio/delete` | POST | Удалить трек, поток или плейлист |
| `/api/audio/metadata` | GET | ICY-метаданные потока (`?url=`) — название и исполнитель |
| `/api/photo/move` | GET | Следующее/предыдущее фото (`pid`, `next`) |

Личные потоки удаляет только владелец; общие станции (`global_streams`) удаляются только в админке.
...
| `/admin?type=RadioStations` | Общие радиостанции сайта (для всех пользователей) |
...
### API админки — радиостанции

| URL | Метод | Описание |
|-----|-------|----------|
| `/api/admin/radio/create` | POST | Добавить общую станцию (`title`, `url`, `sort_order`) |
| `/api/admin/radio/delete` | GET | Удалить общую станцию (`?id=`) |
