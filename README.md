...
>
> Документация: [docs/](docs/) · История изменений: [CHANGELOG.md](CHANGELOG.md) · **Актуальная версия: 1.8** · [заметки](docs/releases/1.8.md) · [1.7](docs/releases/1.7.md) · [1.6](docs/releases/1.6.md) · [1.4](https://github.com/Lanmikeman/nativegallery/releases/tag/release-1.4)
...
**Операционная система**: Ubuntu 20.04+ (рекомендуется **Ubuntu 24.04**), Debian 12+, Windows Server 2019+ / Windows 10+ (IIS)\
**Веб-сервер**: Nginx + PHP-FPM (production), Apache (`.htaccess`), IIS (`web.config`), Caddy, OpenLiteSpeed\
...
Актуально для форка **v1.8** (ветка `main`). Подробный список изменений — [CHANGELOG.md](CHANGELOG.md).

### Что изменилось в 1.8 (кратко)

- **Убрана музыка** — раздел `/music`, плеер в шапке, API `/api/audio/*`, админка радио; миграции `sql_0010`/`sql_0011` удалены из репозитория. Очистка БД: `sqlcore/drop_legacy_tables.sql`.
- **Отключена SPA** — `routing.js` не подключается; навигация с полной перезагрузкой, как в upstream claradex.
- **Комментарии к фото** — восстановлена схема origin (inline submit в `Photo.php`, `comments.js` внизу страницы).
- Подробнее: [docs/releases/1.8.md](docs/releases/1.8.md).
...
- [x] **Перелистывание фото** — `GET /api/photo/move`, стрелки на странице фото
- [x] **Комментарии к фото** — схема origin: submit + AJAX на `/api/photo/comment` (см. 1.8)
- [x] **Кэш статики** — `ng_asset()` (git-hash в URL CSS/JS)
- [x] **Развёртывание** — Ubuntu 24.04 + Nginx + PHP 8.3, `deploy/install-ubuntu-24.04.sh`, `deploy/nginx/nativegallery.conf`
- [x] **Документация** — `docs/configuration.md`, `docs/deployment.md`, `docs/routes.md`, `docs/releases/1.8.md`

### Удалено в 1.8 (ранее было в 1.7)

- [ ] ~~Музыка~~ — `/music`, мини-плеер, `/api/audio/*`, админка `MusicSettings` / `RadioStations` — **полностью удалено**
- [ ] ~~SPA без перезагрузки~~ — `routing.js` **отключён** по умолчанию
...
> **Production** форка: **Ubuntu 24.04 + Nginx**. Пути в скриптах по умолчанию: **`/var/www/nativegallery`**; свой каталог — через **`NG_WEB_ROOT`** (пример: `/mnt/win/nativegallery` — [paths.md](docs/paths.md)). Альтернативы: [deployment-alternatives.md](docs/deployment-alternatives.md) · [windows-iis.md](docs/windows-iis.md) · [manual-install.md](docs/manual-install.md) · [docker.md](docs/docker.md).

## Быстрая установка (Ubuntu 24.04 + Nginx + PHP + MySQL) — рекомендуется
...
- импортирует все SQL-миграции из `sqlcore/` (включая `sql_0006` … `sql_0009`);
...
| `NG_WEB_ROOT` | `/var/www/nativegallery` | Путь к проекту (любой свой; пример: `/mnt/win/nativegallery`) |

## Быстрая установка (Debian 12/13 + Nginx)

На Debian PHP 8.3 ставится из репозитория [packages.sury.org](https://packages.sury.org/php/); БД — MariaDB.

```bash
git clone https://github.com/Lanmikeman/nativegallery.git /tmp/nativegallery
cd /tmp/nativegallery
sudo NG_DOMAIN=example.com NG_DB_PASS=your_password bash deploy/install-debian-12.sh
```

| Веб-сервер | Скрипт |
|------------|--------|
| Nginx | `deploy/install-debian-12.sh` |
| Apache | `deploy/install-debian-12-apache.sh` |

Подробнее: [docs/deployment-alternatives.md](docs/deployment-alternatives.md#debian-12--13-bookworm--trixie).
...
Подробно: [docs/docker.md](docs/docker.md). Образ: `Dockerfile`, БД в `docker-compose.yml`.

## Pterodactyl

Импортируйте egg: [deploy/pterodactyl/egg-nativegallery.json](deploy/pterodactyl/egg-nativegallery.json).  
На ноде соберите образ: `docker build -t ghcr.io/lanmikeman/nativegallery:latest .`  
Инструкция: [docs/pterodactyl.md](docs/pterodactyl.md).

## Ручная установка

Полный список пакетов для Ubuntu, Debian, Apache, Rocky/CentOS: **[docs/manual-install.md](docs/manual-install.md)**.
...
3. Импортируйте SQL-миграции **в указанном порядке**:
   ...
   mysql ngallery < sqlcore/sql_0009.sql
   ```
...
Движок поддерживает Apache 2.4 с `mod_rewrite`. Корневой `.htaccess` настроен для front controller.
...
# замените example.com и путь к проекту
...
На Rocky/AlmaLinux/CentOS Stream 9: `deploy/install-rocky-9-apache.sh` и `deploy/apache/nativegallery-rocky.conf`.

Подробнее: [docs/deployment-alternatives.md](docs/deployment-alternatives.md).

## Windows — IIS (альтернатива)

| Параметр | По умолчанию |
|----------|--------------|
| Корень сайта | `C:\inetpub\nativegallery` |
| URL rewrite | `web.config` в корне репозитория |
| Cron | `deploy\windows\setup-task-scheduler.ps1` |

```powershell
git clone https://github.com/Lanmikeman/nativegallery.git C:\inetpub\nativegallery
cd C:\inetpub\nativegallery
composer install --no-dev
.\deploy\windows\setup-iis.ps1 -WebRoot "C:\inetpub\nativegallery"
```

Подробно: [docs/windows-iis.md](docs/windows-iis.md). Также: **Apache на Windows**, **Caddy**, **OpenLiteSpeed** — [docs/deployment-alternatives.md](docs/deployment-alternatives.md).

## Rocky / AlmaLinux / CentOS Stream 9 (альтернатива)

Для серверов на RHEL-семействе (PHP 8.3 из Remi, MariaDB):

| Веб-сервер | Скрипт |
|------------|--------|
| Nginx | `sudo bash deploy/install-rocky-9.sh` |
| Apache (httpd) | `sudo bash deploy/install-rocky-9-apache.sh` |

Конфиги: `deploy/nginx/nativegallery-rocky.conf`, `deploy/apache/nativegallery-rocky.conf`. Пользователь PHP/cron: `apache`. Учтите SELinux — см. документацию.
...
```bash
cd /путь/к/проекту          # по умолчанию /var/www/nativegallery — см. docs/paths.md
git pull
composer install --no-dev

# примените миграции, которые ещё не выполнялись (последняя — sql_0009):
mysql -u USER -p DATABASE < sqlcore/sql_0009.sql

# обновление с 1.7: удалить таблицы музыки, если они есть:
...
В браузере после обновления: **Ctrl+Shift+R**.

Подробнее: [docs/deployment.md](docs/deployment.md), [docs/releases/1.8.md](docs/releases/1.8.md).
...
| [docs/paths.md](docs/paths.md) | Пути по умолчанию и свой `NG_WEB_ROOT` |
| [docs/manual-install.md](docs/manual-install.md) | Ручная установка, все зависимости по стекам |
| [docs/windows-iis.md](docs/windows-iis.md) | Windows IIS, Apache, Caddy, OpenLiteSpeed |
| [docs/deployment-alternatives.md](docs/deployment-alternatives.md) | Автоскрипты: Debian, Apache, Rocky/CentOS |
| [docs/docker.md](docs/docker.md) | Docker и docker compose |
| [docs/pterodactyl.md](docs/pterodactyl.md) | Egg для Pterodactyl |
| [docs/releases/1.8.md](docs/releases/1.8.md) | Заметки к релизу 1.8 — откат музыки и SPA, комментарии |
| [docs/releases/1.7.md](docs/releases/1.7.md) | Заметки к релизу 1.7 (исторический; музыка убрана в 1.8) |
