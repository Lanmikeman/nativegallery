...
Стабилизация production: откат SPA и музыки, восстановление комментариев по схеме upstream.

### Удалено

- **Музыкальный раздел** (добавлен в 1.7, полностью выпилен):
  - `/music`, мини-плеер в шапке, API `/api/audio/*`;
  - админка `MusicSettings`, `RadioStations`, API радиостанций;
  - `AudioLibrary`, контроллеры `MusicController`, `Api/Audio/*`;
  - статика `music-player.js`, `music-page.js`, `music-player.css`, `playerjs.js`;
  - миграции `sql_0010.sql`, `sql_0011.sql` из репозитория;
  - секция `root.audio` в примерах конфигурации;
  - BBCode `[music]`, смайлы `music.gif` / `music2.gif`, пункт «Музыка» в меню.
- **SPA-навигация** — `routing.js` закомментирован в `LoadHead.php`; переходы с полной перезагрузкой страницы (как в claradex).

### Изменено

- **Комментарии к фото** — схема origin: `comments.js` внизу `Photo.php`, `type="submit"`, inline `$('#f1').submit()` → `/api/photo/comment`.
- **API комментариев** — `Create.php`: `hasUploadedFile()` перед `finfo_file()` (исправление 500 на пустом `filebody`).

### Добавлено

- `sqlcore/drop_legacy_tables.sql` — удаление таблиц `audio_*` на серверах, где успели применить 1.7.
- `sqlcore/rollback_after_7bd5fb2.sql` — откат экспериментальных таблиц сообщений и соцбазы.
- Синяя звезда владельца сервера в профиле (`UserRoleBadge`).
- Legacy: `GET /api/photo/loadnew`, заглушка `get-pub-photos` в `api.php`.

### Исправлено

- Комментарии: кнопка «Добавить комментарий» не отправляла запрос после SPA + переноса скриптов в `<head>`.

## [1.7] — 2026-07-02

Тег `release-1.7`. SPA-навигация, перелистывание фото, улучшения админ-панели.

> **Примечание:** в теге `release-1.7` также был музыкальный раздел (`/music`, `sql_0010`/`sql_0011`). В **1.8** музыка и SPA по умолчанию **удалены/отключены** — см. [1.8](#180--2026-07-05).

### Добавлено

- **Музыка** (`/music`, только для авторизованных) — **удалена в 1.8**:
  - личная библиотека, мини-плеер, API `/api/audio/*`, админка радиостанций;
  - миграции `sql_0010.sql`, `sql_0011.sql`.
- **SPA-навигация** (`static/js/routing.js`) — **отключена в 1.8** (закомментирована в `LoadHead.php`).
