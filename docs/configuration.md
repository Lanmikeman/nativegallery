...
## Удалённые параметры (с 1.8)

Секция `root.audio` в `ngallery.yaml` и блок `"audio"` в `storage/auth-settings.json` **больше не используются** — музыкальный раздел удалён из кода в релизе 1.8. Если блок остался в JSON overlay, его можно удалить вручную.
