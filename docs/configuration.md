...
Управление из админки: `/admin?type=AuthSettings` (только для `admin = 1`). Все инстансы (включая из yaml) можно **редактировать**, **удалять** и **добавлять**; изменения в `storage/auth-settings.json` перекрывают `ngallery.yaml`. При удалении инстанса можно перенести привязки пользователей OpenVK на другой узел.
