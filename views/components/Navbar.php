...
use \App\Services\{Auth, DB, AudioLibrary};
use \App\Models\User;
            ...
            <?php } else { ?>
                <?php
                            if ($user->i('admin') > 0) { 
                                ...
                                ?>
                <li class="mm-pad-right<?= AudioLibrary::isEnabled() ? ' mm-wide ng-music-user-row' : '' ?>">
                    <?php if (AudioLibrary::isEnabled()) {
                        include $_SERVER['DOCUMENT_ROOT'] . '/views/components/MusicPlayer.php';
                    } ?>
