...
Управление из админки: `/admin?type=AuthSettings` (только для `admin = 1`). Переключатели и **дополнительные инстансы** сохраняются в `storage/auth-settings.json` и перекрывают значения из yaml — `ngallery.yaml` можно оставить только для чтения. Инстансы из yaml можно включать/отключать; свои узлы OpenVK добавляются кнопкой «Добавить инстанс» (домен, API, название кнопки).
