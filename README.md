...
Актуально для форка **v1.5**. Подробный список изменений — [CHANGELOG.md](CHANGELOG.md).
...
- [x] **Вход через OpenVK** — openvk.org и vepurovk.xyz, привязка в `/lk/profile?type=OpenVK`
- [x] **Редактирование новостей** — админка `/admin?type=News`, пометка «отредактировано» (`sql_0008`)
- [x] **Админ-панель** — Galleries, EntityEdit, исправлена маршрутизация по `?type=`
...
### Необязательные, но будет неплохо их сделать тоже
  - [ ] Авторизация
    - [ ] Через Telegram
    - [x] Через OpenVK (openvk.org, vepurovk.xyz)
    - [ ] Через ВКонтакте
    - [ ] Через Google
    - [ ] Через Яндекс
    - [ ] Через Twitter
...
При `openvk.enabled: true` в `ngallery.yaml` на `/login` и `/register` появляются кнопки входа через [openvk.org](https://openvk.org) и [vepurovk.xyz](https://vepurovk.xyz).
...
| [docs/releases/1.5.md](docs/releases/1.5.md) | Заметки к релизу 1.5 |
