...
Тег `release-1.7`. Музыкальный раздел, SPA-навигация, общие радиостанции для всех пользователей.

### Добавлено

- **Музыка** (`/music`, только для авторизованных):
  - личная библиотека: загрузка треков, HTTP-потоки, прямые URL, импорт M3U, плейлисты;
  - компактный плеер в шапке (рядом с ником), воспроизведение не прерывается при SPA-переходах;
  - ICY-метаданные эфира (название и исполнитель текущей песни на радио);
  - миграция `sqlcore/sql_0010.sql` — таблицы `audio_tracks`, `audio_streams`, `audio_playlists`, `audio_playlist_items`.
- **Общие радиостанции сайта** (для всех пользователей):
  - админка **Контент → Радиостанции** (`/admin?type=RadioStations`);
  - API: `POST /api/admin/radio/create`, `GET /api/admin/radio/delete`;
  - блок «Радио сайта» на странице `/music` (отдельно от личных потоков);
  - миграция `sqlcore/sql_0011.sql` — таблица `audio_global_streams`.
...
- Перелистывание фото и воспроизведение музыки после SPA-переходов.
- Ложное уведомление «Не удалось начать воспроизведение» при успешном старте потока.
- Автозапуск HTTP-потока через ~2 минуты после паузы (событие `ended` у Icecast).
- Задержка старта радио; прямое воспроизведение вместо обязательного прокси.
